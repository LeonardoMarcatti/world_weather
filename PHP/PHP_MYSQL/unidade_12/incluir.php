...
<?php include_once("_incluir/function.php"); ?>

<?php
    // conferir se a navegacao veio pelo preenchimento do formulario
    if( isset($_POST['nomeproduto']) ) {
        $resultado1 = Publicar($_FILES['foto_grande']);
        $resultado2 = Publicar($_FILES['foto_pequena']);
        ...
        $qInserir = mysqli_query($conection, $inserir);
        if(!$qInserir) {
    ...
    $qCategorias = mysqli_query($conection, $categorias);
    if(!$qCategorias) {
    ...
    $qFornecedores = mysqli_query($conection, $fornecedores);
    if(!$qFornecedores) {
        die("Falha na consulta ao banco");   
    }
?>
